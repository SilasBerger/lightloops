// Reference: https://github.com/GBSL-Informatik/teaching-api/blob/main/src/auth/guard.ts

import { NextFunction, Request, Response } from 'express';
import { AccessMatrix, ClientRole, PUBLIC_ROUTES } from '../routes/authConfig';
import Logger from '../utils/logger';
import { HttpStatusCode } from '../utils/errors/BaseError';

interface AccessRegexRule {
    path: string;
    regex: RegExp;
    weight: number;
    access: {
        methods: ('GET' | 'POST' | 'PUT' | 'DELETE')[];
        clientRoles: ClientRole[];
    }[];
}

const regexFromRoute = (route: string) => {
    const parts = route.toLowerCase().split('/');
    const regex = parts
        .map((part) => {
            if (part.startsWith(':')) {
                return '[^\\/]+';
            }
            return part;
        })
        .join('\\/');
    return new RegExp(`^${regex}$`, 'i');
};

export const PUBLIC_GET_ACCESS = new Set(PUBLIC_ROUTES.filter((route) => !route.includes(':')));

export const PUBLIC_GET_ACCESS_REGEX = PUBLIC_ROUTES.filter((route) => route.includes(':')).map(
    regexFromRoute
);

export const createAccessRules = (accessMatrix: AccessMatrix): AccessRegexRule[] => {
    const accessRules = Object.values(accessMatrix);
    const maxParts = accessRules.reduce((max, accessRule) => {
        return Math.max(max, accessRule.path.split('/').length);
    }, 0);
    const accessRulesWithRegex = accessRules.map((accessRule) => {
        const parts = accessRule.path.split('/');
        let weight = 0;
        const path = parts
            .map((part, idx) => {
                if (part.startsWith(':')) {
                    weight += 2 ** ((maxParts - idx) * 2 - 1);
                    return '[^\\/]+';
                } else {
                    weight += 2 ** ((maxParts - idx) * 2);
                }
                return part;
            })
            .join('\\/');

        const regex = new RegExp(`^${path}`, 'i');
        return {
            ...accessRule,
            path: accessRule.path.toLowerCase(),
            regex: regex,
            weight: weight,
        };
    });
    const rules = accessRulesWithRegex.sort((a, b) => b.weight - a.weight);
    Object.freeze(rules);
    if (process.env.NODE_ENV !== 'test') {
        Logger.info('Access Rules created');
    }
    return rules;
};

const routeGuard = (accessMatrix: AccessRegexRule[]) => {
    return (req: Request, res: Response, next: NextFunction): any => {
        const reqPath = req.path.toLowerCase();
        if (!req.clientRole) {
            if (
                PUBLIC_GET_ACCESS.has(reqPath) ||
                PUBLIC_GET_ACCESS_REGEX.some((regex) => regex.test(reqPath))
            ) {
                next();
                return;
            } else {
                return res.status(HttpStatusCode.FORBIDDEN).json({ error: 'No client role found!' });
            }
        }

        if (!requestHasRequiredAttributes(accessMatrix, req.path, req.method, req.clientRole)) {
            return res
                .status(HttpStatusCode.FORBIDDEN)
                .json({
                    error: `Clients with role '${req.clientRole}' are not authorized to perform this request.`,
                });
        }

        next();
    };
};

/**
 * This method checks if the request has the correct roles, paths and methods
 */
const requestHasRequiredAttributes = (
    accessMatrix: AccessRegexRule[],
    path: string,
    method: string,
    clientRole: ClientRole
) => {
    const accessRules = Object.values(accessMatrix);
    const accessRule = accessRules
        .filter((accessRule) => accessRule.regex.test(path))
        .sort((a, b) => b.path.length - a.path.length)[0];
    if (!accessRule) {
        return false;
    }
    const hasAccess = accessRule.access.some(
        (rule) =>
            rule.clientRoles.includes(clientRole) &&
            rule.methods.includes(method as 'GET' | 'POST' | 'PUT' | 'DELETE')
    );
    Logger.info(
        `${hasAccess ? '✅' : '❌'} Access Rule for '${clientRole}': [${method}:${path}] ${JSON.stringify(accessRule)}`
    );
    return hasAccess;
};

export default routeGuard;
